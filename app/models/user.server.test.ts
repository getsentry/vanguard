...
describe("getUserList", () => {
  ...
  describe("query", () => {
    test("matches email", async () => {
      let result = await getUserList({ query: "foo" });
      expect(result.length).toBe(1);
      expect(result[0].id).toBe(user.id);
    });
    test("matches name", async () => {
      let result = await getUserList({ query: "bar" });
      expect(result.length).toBe(1);
      expect(result[0].id).toBe(user.id);
    });
    test("doesnt match everything", async () => {
      let result = await getUserList({ query: "baz" });
...
describe("updateUser", () => {
  ...
  it("can change name on self", async () => {
    const newUser = await updateUser({ id: user.id, userId: user.id, name: "Fancy" });
  ...
  it("can change picture on self", async () => {
    ...
    expect(newUser).toBeDefined();
    expect(newUser.picture).toBe("data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==");
  });

  it("can change notifyReplies on self", async () => {
    const newUser = await updateUser({ id: user.id, userId: user.id, notifyReplies: false });
    expect(newUser).toBeDefined();
    expect(newUser.notifyReplies).toBe(false);
  });

  it("cannot change admin on self", async () => {
    const newUser = await updateUser({ id: user.id, userId: user.id, admin: true });
    expect(newUser).toBeDefined();
    expect(newUser.admin).toBe(false);
  });

  it("cannot change canPostRestricted on self", async () => {
    const newUser = await updateUser({ id: user.id, userId: user.id, canPostRestricted: true });
    expect(newUser).toBeDefined();
...
describe("createUser", () => {
  it("creates a new user", async () => {
    const newUser = await createUser({ email: "foo@example.com", name: "Fancy" });
    expect(newUser).toBeDefined();
